...
$.currentWindow = function () {
    var iframeId = top.$(".Finix_iframe:visible").attr("id");
        ...
        $.ajax({
            ...
            success: function (data) {
                $.each(data, function (i) {
                    $element.append(
                        $("<option></option>")
                        .val(data[i][options.id])
                        .html(data[i][options.text])
                        .attr("price", data[i][options.price])
                        .attr("draw", data[i][options.draw])
                        .attr("num", data[i][options.num])
                        .attr("orderPrice", data[i][options.orderPrice])
                        .attr("deliveryDate", data[i][options.deliveryDate])
                        .attr("orderAmount", data[i][options.orderAmount])
                        .attr("orderStatus", data[i][options.orderStatus])
                        .attr("customerCode", data[i][options.customerCode])
                        .attr("productName", data[i][options.productName])
                        );
                });
                $element.select2({
...
$.fn.authorizeButton = function () {
    var moduleId = top.$(".Finix_iframe:visible").attr("id").substr(6);
